...
from live_runtime_rig_nexus_kernelized.cases import register_cases
...
def _case_executed(case: Any, run_root: Path, checks: list[dict[str, Any]]) -> bool:
    ...
    if direct.is_file():
        return True
    # Kernelized target suites are one case per suite. This also catches a case
    # that started but raised before write_case() could persist its case record.
...
def print_report(report: dict[str, Any]) -> None:
    ...
    if items:
        print()
        print("TESTS NOT COMPLETED BECAUSE THE CHAIN BROKE")
        for item in items:
            print(f"  [NOT RUN] {item['suite']} :: {item['name']}")
    else:
        print("All planned kernelized acceptance stages were reached.")
